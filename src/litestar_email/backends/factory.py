...
Public API: ``get_backend``, ``get_backend_class``, ``list_backends``,
``email_backend``. These are re-exported from
:mod:`litestar_email.backends`.
"""
...
def email_backend(name: str) -> "type[BaseEmailBackend]":
